...
class SSOAuthenticator(Authenthicator):
    idxName = "SSOIdentities"
    id = 'SSO'
    name = 'SSO'
    ...
    def __init__(self):
        ...
        self.UserCreator = SSOUserCreator()

    def createIdentity(self, li, avatar):
        Logger.get("auth.ldap").info("createIdentity %s (%s %s)" % \
                                     (li.getLogin(), avatar.getId(),
                                      avatar.getEmail()))
        if SSOChecker().check(li.getLogin(), li.getPassword()):
            return SSOIdentity(li.getLogin(), avatar)
        else:
            return None


class SSOIdentity(PIdentity):

    def __str__(self):
        return '<SSOIdentity{login:%s, tag:%s}>' % \
    ...
    def authenticate(self, id):
        ...
        data = SSOChecker().check(id.getLogin(), id.getPassword())
        if data:
            if self.getLogin() == id.getLogin():
                # modify Avatar with the up-to-date info from SSO
                av = self.user

                av.clearAuthenticatorPersonalData()

                if 'postalAddress' in data:
                    postalAddress = fromSSOmultiline(data['postalAddress'])
...
class SSOConnector(object):
    """
    handles communication with the SSO server specified in indico.conf
    ...
    the code expects the users to be (in the SSO) objects of type inetOrgPerson
    ...
    of users which seems to be the standard SSO setup
    """

    def __init__(self):
        conf = Configuration.Config.getInstance()
        ldapConfig = conf.getSSOConfig()
    ...
    def login(self):
        try:
            ...
            raise Exception("Cannot login to SSO server")
    ...
    def open(self):
        """
        Opens an anonymous SSO connection
        """
    ...
    def openAsUser(self, userName, password):
        """
        Verifies username and password by binding to the SSO server
        """
        self.open()
        self.login()
    ...
    def create(self, li):
        ...
        if len(userList) == 0:
            ...
            try:
                ...
                if 'postalAddress' in data:
                    av.setAddress(fromSSOmultiline(data.get('postalAddress')))
                #av.setTelephone(data.get('telephonenumber',""))
                ah.add(av)
                av.activateAccount()
            except KeyError:
                raise MaKaCError("SSO account does not contain the mandatory"
...
def dictToAv(ret):
    ...
    if 'postalAddress' in ret:
        av['address'] = [fromSSOmultiline(ret['postalAddress'])]
...
class SSOUser(object):
    ...
    def getById(self, id):
        ...
        av = dictToAv(ret)
        av["id"] = id
        av["identity"] = SSOIdentity
        av["authenticator"] = SSOAuthenticator()
        return av
